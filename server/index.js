...
const GOOGLE_SHEETS_ENABLED = Boolean(GOOGLE_SHEETS_ID && GOOGLE_SERVICE_ACCOUNT_EMAIL && GOOGLE_PRIVATE_KEY);

// Записує рядок у Google Таблицю в фоні (Час, Email, Країна). Не блокує
// реєстрацію і не ламає її, якщо інтеграція не налаштована або Google API
// тимчасово недоступний.
function syncToGoogleSheet(email, country) {
  ...
  sheets
    .appendRow({
      ...
      values: [new Date().toISOString(), email, country],
    })
    .catch((err) => {
...
app.post('/api/register', (req, res) => {
  ...
  // Приймаємо або одну з попередньо заданих країн, або довільну назву,
  // вписану вручну при виборі "Інша" (сама позначка "Інша" неприпустима —
  // фронтенд завжди підміняє її на введений текст).
  const trimmedCountry = country.trim();
  if (!trimmedCountry || trimmedCountry.length > 60 || trimmedCountry === 'Інша') {
    return res.status(400).json({ error: 'invalid_country', message: 'Вкажи країну.' });
  }
  ...
  db.insert({ token, email: normalizedEmail, country: trimmedCountry, ip });
  syncToGoogleSheet(normalizedEmail, trimmedCountry);
  ...
  res.json({ ok: true, email: normalizedEmail, country: trimmedCountry });
});
